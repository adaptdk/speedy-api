...
// CREATE Profile 
SpeedTracker.prototype.createProfile = function (content) {
  const { name } = content
  ...
  return this._getRemoteFile(path).then(data => {
    try {
      return Promise.reject(Utils.buildError('Profile already exists'))
    } catch (err) {
      return Promise.reject(Utils.buildError('CORRUPT_RESULT_FILE'))
    }
  }).catch(err => {
    if (err.code === 404) {
      ...
      const formattedContent = YAML.stringify(payload);
      ...
      return Promise.reject(Utils.buildError('Something went wrong'))
    }
  })
}
